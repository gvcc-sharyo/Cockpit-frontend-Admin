...
const StudentChapter = () => {
  ...
  useEffect(() => {
    const fetchBooksAndChapters = async () => {
      try {
        ...
        const chapterResponse = await apiGet(
          `/chaptersBySyllabusId/${syllabusId}`
        );
  ...
  const getStudentProgress = async () => {
    try {
      const response = await apiPost(`/studentTaskProgress`, {
        studentId: studentId,
      });
  ...
  useEffect(() => {
    const filterChapters = chapters.filter(
      (chapter) => chapter.bookId === selectedBookID
    );
          ...
          {books?.map((book, index) => (
            ...
            >
              <Button
                ...
                sx={{
                  backgroundColor:
                    selectedBookID === book?._id ? "#f5f5f5" : "#0f2848",
                  ...
                  "&:hover": {
                    backgroundColor:
                      selectedBookID === book?._id ? "#eaeaea" : "#16355c", // Optional hover bg
